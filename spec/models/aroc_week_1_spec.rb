...
# ----------------------
  ...
  it '4. finds orders of multiple amounts' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # orders_of_500_and_700 = Order.where('amount = 500 OR amount = 700') # this is SQL syntax (quotes)
    # orders_of_700_and_1000 = Order.where('amount = 700 OR amount = 1000')

    orders_of_500_and_700 = Order.where(amount: [500, 700]) # this is AR syntax [brackets with comma]
  ...
  it '6. finds multiple orders by id' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    orders = Order.where(id: [@order_1.id, @order_3.id, @order_5.id, @order_7.id])
    # ------------------------------------------------------------
  ...
  it '8. finds orders with an amount less than 550' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # orders_less_than_550 = Order.where('amount < 550') #SQL
    # orders_less_than_550 = Order.where(amount: [0..549]) #AR
    orders_less_than_550 = Order.where(amount: [Float::INFINITY..549]) #AR??
    # ------------------------------------------------------------
